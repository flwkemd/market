		...
		<div class="table-board">
			...
			<colgroup>
				...
				<%
	        		...
	        		ArrayList<Store3Bean> list = store3DAO.getBoardList(start);
	        		for(int i=0; i<list.size(); i++){
				%>
				
				<tr align="center">
					<td><%= list.get(i).getStore3Id() %></td>
					<td><%= list.get(i).getStore3Form() %></td>
					<td><%= list.get(i).getStore3Title()%></td>
					<td><a href="store3Detail.do?store3Id=<%= list.get(i).getStore3Id() %>" class="view">상세보기</a></td>
				</tr>
  		 	...
  		 	<div id="pageForm">
				<%
					if(startPage != 1){
				%>	
					<a href='store3.do?page=<%=startPage %>' id="numBtn">[이전]</a>	
				<%
					}
				...
				%>
						<a href="store3.do?page=<%= i %>" id="numBtn"><%= i %>&nbsp;</a>
				<%			
						}
					}
				%>
				...
				<%
					if(endPage != maxPage){
				%>
					<a href='store3.do?page=<%=endPage %>' id="numBtn">[다음]</a>	
				<%		
					}
				%>
			</div>	
