		...
		<div class="table-board">
			...
			<colgroup>
				...
				<%
	        		...
	        		ArrayList<Store2Bean> list = store2DAO.getBoardList(start);
	        		for(int i=0; i<list.size(); i++){
				%>
				
				<tr align="center">
					<td><%= list.get(i).getStore2Id() %></td>
					<td><%= list.get(i).getStore2Form() %></td>
					<td><%= list.get(i).getStore2Title()%></td>
					<td><a href="store2Detail.do?store2Id=<%= list.get(i).getStore2Id() %>" class="view">상세보기</a></td>
				</tr>
  		 	...
  		 	<div id="pageForm">
				<%
					if(startPage != 1){
				%>	
					<a href='store2.do?page=<%=startPage %>' id="numBtn">[이전]</a>	
				<%
					}
				...
				%>
						<a href="store2.do?page=<%= i %>" id="numBtn"><%= i %>&nbsp;</a>
				<%			
						}
					}
				%>
				...
				<%
					if(endPage != maxPage){
				%>
					<a href='store2.do?page=<%=endPage %>' id="numBtn">[다음]</a>	
				<%		
					}
				%>
			</div>	
