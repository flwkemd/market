<%@page import="jsp.store8.model.Store8Bean"%>
<%@page import="jsp.store8.model.Store8DAO"%>
...
<head>
  ...
  <body id="page-top">
	...
	<%
		...
		int store8Id =1;
		if(request.getParameter("store8Id") !=null){
			store8Id = Integer.parseInt(request.getParameter("store8Id"));
		}
    ...
    <div class="content">
      <div class="container">
        <div class="row">
          <div class="col-lg-12">
          	<p class="sub-title text-center">기타 상점별 안내</p>
          	<p class="sub-font1 text-center">면목시장은 찾아오시는 고객분들을 위한<br>다양한 편의시설이 준비되어 있습니다.</p>
          	<%
            	if(id!=null){
            %>
            <h3 class="section-heading text-muted" align="right"><a href="store8/store8Write.jsp" class="click_a">글쓰기<span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span></a></h3>
          	<%
            	}
          	%>
            <hr>
			 ...
			 <%
				Store8DAO store8DAO = new Store8DAO();
			 	
      			Store8Bean store = store8DAO.getDetail(store8Id);
      	     %>
        
			<div class="col-lg-12 store-item">
              <img class="img-fluid" src="../makeStore/UploadFolder/Store8/<%= store.getStore8File1() %>">
            
            <div class="store-caption">
              <h1><%= store.getStore8Title()%></h1>
              <hr>
            <div class="info">
				<dl class="infolist">
					<dt class="item_title">
						...
						<dd class="item_content"> <span class="text">&nbsp;&nbsp;<%= store.getStore8Num() %></span></dd>
				</dl>
				<hr>
				<dl class="infolist">
					<dt class="item_title">
					<span class="glyphicon glyphicon-time" aria-hidden="true"></span>영업시간</dt>
						<dd class="item_content"> <span class="text">평일 : </span><%=store.getStore8Time1() %></dd>
						<dd class="item_content"> <span class="text">주말 : </span><%=store.getStore8Time2() %></dd>
						<dd class="item_content"> <span class="text">휴일 : </span><%=store.getStore8Holiday() %></dd>
				</dl>
				<hr>
				<dl class="infolist">
					<dt class="item_title">
					<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>서비스</dt>
						<dd class="item_content"> <span class="text">온누리상품권 : </span><%=store.getStore8Service1() %></dd>
						<dd class="item_content"> <span class="text">배달서비스 : </span><%=store.getStore8Service2() %></dd>
						<dd class="item_content"> <span class="text">카드결제 : </span><%=store.getStore8Service3() %></dd>
				</dl>
				<hr>
				
				<p id="mapText"> 지도에서 위치 보기</p>
					<div class="col-lg-12">
						 <img class="img-fluid" src="../makeStore/UploadFolder/Store8/<%= store.getStore8File2() %>">
					</div>
	             	  </div>
           		   </div>
    	       </div>
