		...
		<div class="table-board">
			...
			<colgroup>
				...
				<%
	        		...
	        		ArrayList<Store8Bean> list = store8DAO.getBoardList(start);
	        		for(int i=0; i<list.size(); i++){
				%>
				
				<tr align="center">
					<td><%= list.get(i).getStore8Id() %></td>
					<td><%= list.get(i).getStore8Form() %></td>
					<td><%= list.get(i).getStore8Title()%></td>
					<td><a href="store8Detail.do?store8Id=<%= list.get(i).getStore8Id() %>" class="view">상세보기</a></td>
				</tr>
  		 	...
  		 	<div id="pageForm">
				<%
					if(startPage != 1){
				%>	
					<a href='store8.do?page=<%=startPage %>' id="numBtn">[이전]</a>	
				<%
					}
				...
				%>
						<a href="store8.do?page=<%= i %>" id="numBtn"><%= i %>&nbsp;</a>
				<%			
						}
					}
				%>
				...
				<%
					if(endPage != maxPage){
				%>
					<a href='store8.do?page=<%=endPage %>' id="numBtn">[다음]</a>	
				<%		
					}
				%>
			</div>	
