...
<head>
  ...
  <body id="page-top">
	...
	<%
		...
		int store6Id =1;
		if(request.getParameter("store6Id") !=null){
			store6Id = Integer.parseInt(request.getParameter("store6Id"));
		}
            ...
            <hr>
			 ...
			 <%
				Store6DAO store6DAO = new Store6DAO();
      			...
      			Store6Bean store = store6DAO.getDetail(store6Id);
      	     %>
        
			<div class="col-lg-12 store-item">
              <img class="img-fluid" src="../makeStore/UploadFolder/Store6/<%= store.getStore6File1() %>">
            
            <div class="store-caption">
              <h1><%= store.getStore6Title()%></h1>
              <hr>
            <div class="info">
				<dl class="infolist">
					<dt class="item_title">
						...
						<dd class="item_content"> <span class="text">&nbsp;&nbsp;<%= store.getStore6Num() %></span></dd>
				</dl>
				<hr>
				<dl class="infolist">
					<dt class="item_title">
					<span class="glyphicon glyphicon-time" aria-hidden="true"></span>영업시간</dt>
						<dd class="item_content"> <span class="text">평일 : </span><%=store.getStore6Time1() %></dd>
						<dd class="item_content"> <span class="text">주말 : </span><%=store.getStore6Time2() %></dd>
						<dd class="item_content"> <span class="text">휴일 : </span><%=store.getStore6Holiday() %></dd>
				</dl>
				<hr>
				<dl class="infolist">
					<dt class="item_title">
					<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>서비스</dt>
						<dd class="item_content"> <span class="text">온누리상품권 : </span><%=store.getStore6Service1() %></dd>
						<dd class="item_content"> <span class="text">배달서비스 : </span><%=store.getStore6Service2() %></dd>
						<dd class="item_content"> <span class="text">카드결제 : </span><%=store.getStore6Service3() %></dd>
				</dl>
				<hr>
				
				<p id="mapText"> 지도에서 위치 보기</p>
					<div class="col-lg-12">
						 <img class="img-fluid" src="../makeStore/UploadFolder/Store6/<%= store.getStore6File2() %>">
					</div>
	             	  </div>
           		   </div>
    	       </div>
